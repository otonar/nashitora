...
def main():
    ...
    if not api_key or not openai.api_key:
        print("Error: 必要なAPIキーが環境変数に設定されていません")
        return

    # CSVファイルからロケーション情報を読み込む
    locations = load_locations('location_list.csv')

    # 目的地の番号を読み込む
    destination_numbers = load_destination_numbers('destination_no.csv')

    # 選択された目的地の情報を取得
    waypoints = get_selected_waypoints(locations, destination_numbers)

    if not waypoints:
        print("有効な目的地が見つかりませんでした。")
        return
    ...
    # HTMLファイルを作成
    with open("route_map5.html", "w", encoding="utf-8") as file:
        file.write(html_content)

    print("HTMLファイル 'route_map5.html' を作成しました。ブラウザで開いてください。")
